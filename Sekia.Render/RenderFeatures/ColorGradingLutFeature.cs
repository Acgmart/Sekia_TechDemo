...
namespace Sekia
{
    public class ColorGradingLutFeature : ScriptableRendererFeature
    {
        public class FeaturePass : ScriptableRenderPass
        {
            ...
            public FeaturePass(FeatureSettings settings)
            {
                ...
                m_LutBuilderLdr = Load(settings.data.shaders.lutBuilderLdrPS);
                m_LutBuilderHdr = Load(settings.data.shaders.lutBuilderHdrPS);
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                RenderTextureDescriptor desc = new RenderTextureDescriptor(lutWidth, lutHeight, format, 0);
                desc.vrUsage = VRTextureUsage.None;
                FilterMode filterMode = FilterMode.Bilinear;

                RenderingUtils.ReAllocateIfNeeded(ref ScriptableRendererFeature.colorGradingLut, desc, filterMode, TextureWrapMode.Clamp, name: "_InternalGradingLut");

                m_InternalLut = GetRT(settings.dstLut);
            }
        ...
        {
            public string profilerTag = "ColorGradingLutFeature";
            public PostProcessData data = null;
            public RT_T0 dstLut = RT_T0.PP_Lut;
        }
        ...
        public override void Create()
        {
            ...
            if (settings.data == null)
                Debug.LogError("PostProcessData不能为空");
