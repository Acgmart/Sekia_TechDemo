...
namespace Sekia
{
    ...
    public class PostProcessFeature : ScriptableRendererFeature
    {
        public class FeaturePass : ScriptableRenderPass
        {
            ...
            bool m_IsFinalPass = false; //当前pass是否为FinalPostProcessing
            ...
            bool m_HasFinalPass = false; //判断是否执行Grain|Dither等在FXAA后执行的pass
            ...
            public FeaturePass(FeatureSettings settings)
            {
                ...
                m_Data = settings.postProcessData;
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                m_Source = GetRT(settings.srcColor);
                m_Depth = GetRT(settings.srcDepth);
                m_InternalLut = GetRT(settings.srcLut);

                m_EnableSRGBConversionIfNeeded = settings.renderToBackBuffer;
                m_ResolveToScreen = settings.renderToBackBuffer;
                m_Destination = ScriptableRendererFeature.k_CameraTarget;
            ...
            void Render(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                RTHandle destination = m_UseSwapBuffer ? renderer.GetCameraColorFrontBuffer() : ScriptableRendererFeature.k_CameraTarget;
                ...
                void Swap(ref ScriptableRenderer r)
                {
                    ...
                    if (m_UseSwapBuffer)
                    {
                        ...
                        destination = r.GetCameraColorFrontBuffer();
                    }
                    else
                    {
                ...
                using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
                {
                    ...
                    if (m_Destination == ScriptableRendererFeature.k_CameraTarget && !cameraData.isDefaultViewport)
...
#pragma warning restore 0618
            ...
            void DoSubpixelMorphologicalAntialiasing(ref CameraData cameraData, CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination)
            {
                ...
                if (m_Depth == ScriptableRendererFeature.k_CameraTarget || m_Descriptor.msaaSamples > 1)
                {
        ...
        internal static readonly Matrix4x4[] viewProjMatrixStereo = new Matrix4x4[2];
            ...
            void SetupColorGrading(CommandBuffer cmd, ref RenderingData renderingData, Material material)
            {
                ...
                else
                {
                    switch (m_Tonemapping.mode.value)
                    {
                        ...
                        case TonemappingMode.ACES_1: material.EnableKeyword(ShaderKeywordStrings.TonemapACES_1); break;
                        case TonemappingMode.ACES_2: material.EnableKeyword(ShaderKeywordStrings.TonemapACES_2); break;
                        default: break; // None
                    }
                }
            }
            ...
            void RenderFinalPass(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                if (m_UseSwapBuffer)
                    m_Source = cameraData.renderer.GetCameraColorBackBuffer();
...
#pragma warning restore 0618
        ...
        {
            public string profilerTag = "PostProcessFeature";
            public PostProcessData postProcessData = null;
            public Material blitMaterial = null;
            public bool renderToBackBuffer = false;
            public RT_T0 srcColor = RT_T0.ColorMain;
            public RT_T0 srcDepth = RT_T0.DepthMain;
            public RT_T0 srcLut = RT_T0.PP_Lut;
        }
