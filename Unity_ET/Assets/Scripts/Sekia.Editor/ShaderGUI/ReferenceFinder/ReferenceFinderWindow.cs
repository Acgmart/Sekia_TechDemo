...
//https://github.com/networm/ReferenceFinder
...
namespace Sekia
{
    public class ReferenceFinderWindow : EditorWindow
    {
        ...
        //查找资源引用信息
        //%#&f
        [MenuItem("Assets/TA工具/Find References In Project &f", false, 25)]
        static void FindRef()
        {
            InitDataIfNeeded();
        ...
        public void DrawOptionBar()
        {
            ...
            if (GUILayout.Button("Collapse", toolbarButtonGUIStyle))
            {
                if (m_AssetTreeView != null)
                {
                    ...
                    if (m_AssetTreeView.assetRoot != null &&
                    m_AssetTreeView.assetRoot.children != null &&
                    m_AssetTreeView.assetRoot.children.Count != 0)
                        m_AssetTreeView.SetExpanded(m_AssetTreeView.assetRoot.children[0].id, true);
                }
            }
        ...
        //生成root相关
        public static HashSet<string> updatedAssetSet = new HashSet<string>();
