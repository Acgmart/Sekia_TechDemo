...
//在Unity启动和按下Run时会调用其[构造函数] 根据后缀加载文件面板
[InitializeOnLoad]
public class InspectorBase : Editor
{
    static InspectorBase()
    {
        EditorApplication.update += Update; //添加委托函数到update集合
    }

    static void Update()
    {
        if (Selection.objects.Length == 1) //已选择单个文件
        {
            string path = AssetDatabase.GetAssetPath(Selection.activeObject);
            string extension = Path.GetExtension(path).ToLower();

            //判断文件后缀
            if (AnimInspector.inspectorSwitch && extension == ".anim")
            {
                //Inspector按钮关闭 或 菜单栏Window/ Custom AnimInspector开启
                SetupScriptableObject<AnimScriptableObject>(Selection.activeObject);
            }
        }
    }

    //选择指定后缀文件时执行一次
    static void SetupScriptableObject<T>(Object target) where T : CustomScriptableObject
    {
        if (target as T == null) //新被选择的文件
        {
            //内存中创建实例
            T scriptableObject = ScriptableObject.CreateInstance<T>();
            scriptableObject.asset = target;
            Selection.activeObject = scriptableObject; //设置“当前选择目标”
        }
    }
}

public class AnimInspectorWindow : EditorWindow
{
    [MenuItem("Window/Turn On AnimInspector")]
    public static void Init()
    {
        AnimInspector.inspectorSwitch = true;
    }
}

//.anim文件自定义序列化文件
public class AnimScriptableObject : CustomScriptableObject { }

//.anim文件自定义面板
[CustomEditor(typeof(AnimScriptableObject))]
public class AnimInspector : Editor
{
    //存储面板参数
    string animMessage = "";
    ...
    public static bool inspectorSwitch = false;

    //面板状态变为可编辑(Enabled)时执行一次
    private void OnEnable()
    {
        AnimScriptableObject anim = (AnimScriptableObject)target;
        customAnim = new CustomAnim(anim.asset); //解析Clip文件数据
    }

    public override void OnInspectorGUI() //每帧执行
    {
        //base.OnInspectorGUI(); //绘制父级CustomScriptableObject的GUI
        GUI.enabled = !EditorApplication.isPlaying; //运行时面板不可编辑
        ...
        if (fileModified)
        {
            precision = EditorGUILayout.Slider("Precision", precision, 0F, 0.1F);
            customAnim.precision = precision;

            if (GUILayout.Button("Delete unused KeyFrames"))
            {
                customAnim.DeleteUnusedKeyFrames();
                animMessage = "Delete unused KeyFrames succeed!\nSee new file " + customAnim.animName + "_fixed.anim for detail.";
                fileModified = false;
            }
        }

        GUILayout.Space(40);
        if (GUILayout.Button("Turn off AnimInspector")) //关闭自定义面板 可在Window/ Custom AnimInspector开启
        {
            Selection.activeObject = customAnim.clip;
...
public class CustomAnim
{
    ...
    public void DeleteUnusedKeyFrames()
    {
        ...
        string exportFilePath = "Assets" + animFilePath + result.name + ".anim";
