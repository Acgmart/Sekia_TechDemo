...
public class FlyingBirds : MonoBehaviour {
    ...
    void Update () {
        ...
        pause = Input.GetKeyDown(KeyCode.P) ? !pause : pause;
        if(!pause)
            UpdateBirds();
        mpb.SetMatrix("_O2W", transform.localToWorldMatrix);
        mpb.SetFloat("_targetSize", targetSize);
        mpb.SetFloat("_Adjust", adjust);
        Graphics.DrawMeshInstancedIndirect(mesh, 0, mat, bounds, argBuffer, 0, mpb, ShadowCastingMode.On);
	}
    [System.Serializable]
    public struct Bird
    {
