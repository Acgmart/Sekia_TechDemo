...
#define ts _PosTex_TexelSize
	...
	float _Smooth, _Length, _DT, _targetSize, _Adjust;
	...
	{
		float t = (_Time.y - _DT) / _Length - 10 * rand(iid*0.01);
#if ANIM_LOOP
		t = fmod(t, 1.0);
#else
		t = saturate(t);
		...
		float3 wPos = b.pos;
		float4 rot = b.rot;
		float dst = distance(wPos, _PosBuffer[iid] * _targetSize);

		pos = rotateWithQuaternion(pos, rot);
		pos += lerp(wPos, _PosBuffer[iid] * _targetSize, _Adjust);
		normal = rotateWithQuaternion(normal, rot);
		pos = mul(_O2W, float4(pos, 1)).xyz;
		...
		out half4 outEmission : SV_Target3)
	{
		...
		outEmission = _Emission / max(0.01, i.dst*i.dst);
	}
