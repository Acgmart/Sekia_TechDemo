...
namespace IndieVisualLab.FrontCover2
{
    ...
    public class GPUVoxelParticleSystem : MonoBehaviour {
        ...
        [SerializeField, Range(0.5f, 3f)] protected float lifetime = 1.5f;
        [SerializeField, Range(0.0f, 1f)] protected float size = 0.5f;

        #endregion

        #region Interaction properties

        [SerializeField, Range(0.0f, 100f)] protected float radius = 10f;
        [SerializeField, Range(0.0f, 0.1f)] protected float force = 0.05f;
        ...
        protected void Start () {
            ...
            gradientTexture = gradient.Create(128, 1);
            Setup();
        }
        ...
        protected void Update () {
            ...
            block.SetMatrix("_WorldToLocal", transform.worldToLocalMatrix);
            block.SetMatrix("_LocalToWorld", transform.localToWorldMatrix);
            Graphics.DrawMeshInstancedIndirect(cube, 0, material, new Bounds(Vector3.zero, Vector3.one * 100f), argsBuffer, 0, block, castShadows, receiveShadows);
        }
        ...
        void Setup()
        {
            ...
            particleCompute.SetTexture(setupKer.Index, "_Gradient", gradientTexture);
            ...
            particleCompute.SetVector("_BoundsMin", bounds.min);
            particleCompute.SetVector("_BoundsMax", bounds.max);
        ...
        void Compute (Kernel kernel, float dt)
        {
            ...
            particleCompute.SetFloat("_Size", size);
            particleCompute.SetTexture(kernel.Index, "_Gradient", gradientTexture);
            particleCompute.SetVector("_BoundsMin", bounds.min);
            particleCompute.SetVector("_BoundsMax", bounds.max);

            var cam = Camera.main;
            var input = Input.mousePosition;
            input.z = Mathf.Abs(transform.position.z - cam.transform.position.z);
            var world = cam.ScreenToWorldPoint(input);
            var mouse = transform.InverseTransformPoint(world);
            particleCompute.SetVector("_Point", mouse);
            particleCompute.SetFloat("_Radius", 1f / radius);
            particleCompute.SetFloat("_Force", force);

            var mv = cam.worldToCameraMatrix * transform.localToWorldMatrix;
            particleCompute.SetMatrix("_MV", mv);
            particleCompute.SetMatrix("_IMV", mv.inverse);
