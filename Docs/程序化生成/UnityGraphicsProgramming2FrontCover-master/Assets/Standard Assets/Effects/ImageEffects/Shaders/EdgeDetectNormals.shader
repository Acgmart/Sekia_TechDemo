...
Shader "Hidden/EdgeDetect" { 
	...
	{	
		...
		return lerp(lerp(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv[0].xy, _MainTex_ST)), _BgColor, _BgFade), _EdgesColor, Sobel);
	}
	...
	half4 fragRobert(v2f i) : SV_Target {				
		...
		// return lerp(lerp(col, _BgColor, _BgFade), _EdgesColor, saturate(1.0 - edge));
		return lerp(col, lerp(col, _EdgesColor, 0.5), saturate(1.0 - edge));
	}
