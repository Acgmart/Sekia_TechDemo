    ...
    internal class LightCookieManager : IDisposable
    {
        ...
        private struct Sorting
        {
            ...
            public static void QuickSort<T>(T[] data, int start, int end, Func<T, T, int> compare)
            {
                ...
                UnityEngine.Assertions.Assert.IsTrue((uint)start < data.Length);
                UnityEngine.Assertions.Assert.IsTrue((uint)end < data.Length); // end == inclusive
            ...
            {
                UnityEngine.Assertions.Assert.IsTrue((uint)start < data.Length);
                UnityEngine.Assertions.Assert.IsTrue((uint)end < data.Length);
        ...
        private readonly struct WorkSlice<T>
        {
            ...
            public WorkSlice(T[] src, int srcStart, int srcLen = -1)
            {
                ...
                UnityEngine.Assertions.Assert.IsTrue(m_Start + m_Length <= capacity);
            }
        ...
        int FilterAndValidateAdditionalLights(ref LightData lightData, LightCookieMapping[] validLightMappings)
        {
            ...
            for (int i = 0; i < visibleLightCount; i++)
            {
                ...
                UnityEngine.Assertions.Assert.IsTrue(i < ushort.MaxValue);
        ...
        int FetchUVRects(CommandBuffer cmd, ref WorkSlice<LightCookieMapping> validLightMappings, Vector4[] textureAtlasUVRects, int cookieSizeDivisor)
        {
            ...
            for (int i = 0; i < validLightMappings.length; i++)
            {
                ...
                {
                    UnityEngine.Assertions.Assert.IsTrue(light.type == LightType.Point);
                    uvScaleOffset = FetchCube(cmd, cookie, cookieSizeDivisor);
                }
                else
                {
                    UnityEngine.Assertions.Assert.IsTrue(light.type == LightType.Spot || light.type == LightType.Directional, "Light type needs 2D texture!");
        ...
        {
            UnityEngine.Assertions.Assert.IsTrue(cookie != null);
            UnityEngine.Assertions.Assert.IsTrue(cookie.dimension == TextureDimension.Tex2D);
        ...
        {
            UnityEngine.Assertions.Assert.IsTrue(cookie != null);
            UnityEngine.Assertions.Assert.IsTrue(cookie.dimension == TextureDimension.Cube);
