...
namespace Sekia
{
    public class Shadow_Additional_Feature : ScriptableRendererFeature
    {
        public class FeaturePass : ScriptableRenderPass
        {
            ...
            int MAX_PUNCTUAL_LIGHT_SHADOW_SLICES_IN_UBO  // keep in sync with MAX_PUNCTUAL_LIGHT_SHADOW_SLICES_IN_UBO in Shadows.hlsl
            {
                get
                {
                    if (SekiaPipeline.maxVisibleAdditionalLights != SekiaPipeline.k_MaxVisibleAdditionalLightsNonMobile)
                        ...
                        return SekiaPipeline.maxVisibleAdditionalLights;
                    else
            ...
            public FeaturePass(FeatureSettings settings)
            {
                ...
                int maxVisibleAdditionalLights = SekiaPipeline.maxVisibleAdditionalLights;
                const int maxMainLights = 1;
                int maxVisibleLights = SekiaPipeline.maxVisibleAdditionalLights + maxMainLights;
                int maxAdditionalLightShadowParams = m_UseStructuredBuffer ? maxVisibleLights : Math.Min(maxVisibleLights, SekiaPipeline.maxVisibleAdditionalLights);
            ...
            public bool Setup(ref RenderingData renderingData)
            {
                ...
                int maxAdditionalLightShadowParams = m_UseStructuredBuffer ? visibleLights.Length : Math.Min(visibleLights.Length, SekiaPipeline.maxVisibleAdditionalLights);
            ...
            void RenderAdditionalShadowmapAtlas(ref ScriptableRenderContext context, ref CullingResults cullResults, ref LightData lightData, ref ShadowData shadowData)
            {
                ...
                using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.AdditionalLightsShadow)))
                {
                    ...
                    for (int globalShadowSliceIndex = 0; globalShadowSliceIndex < shadowSlicesCount; ++globalShadowSliceIndex)
                    {
                        ...
                        settings.useRenderingLayerMaskTest = SekiaPipeline.asset.supportsLightLayers;
        ...
        {
            public string profilerTag = "Shadow_Additional_Feature";
        }
        ...
        public override void Create()
        {
            ...
            m_AdditionalLightsShadowCasterPass = pass;
        }

        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            applyAdditionalShadow = pass.Setup(ref renderingData);
            if (applyAdditionalShadow)
